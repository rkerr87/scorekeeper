...
describe('PlayDetailPopover', () => {
  ...
  it('calls onUndo directly when no subsequent plays', async () => {
    const onUndo = vi.fn()
    const user = userEvent.setup()
    render(<PlayDetailPopover play={play} playsAfterCount={0} onEdit={vi.fn()} onUndo={onUndo} onClose={vi.fn()} />)
  ...
  it('shows confirmation before undo when subsequent plays exist', async () => {
    const onUndo = vi.fn()
    const user = userEvent.setup()
    render(<PlayDetailPopover play={play} playsAfterCount={3} onEdit={vi.fn()} onUndo={onUndo} onClose={vi.fn()} />)
    await user.click(screen.getByRole('button', { name: /undo/i }))
  ...
  it('can cancel undo confirmation', async () => {
    const onUndo = vi.fn()
    const user = userEvent.setup()
    render(<PlayDetailPopover play={play} playsAfterCount={2} onEdit={vi.fn()} onUndo={onUndo} onClose={vi.fn()} />)
    await user.click(screen.getByRole('button', { name: /undo/i }))
    expect(screen.getByText(/will also remove 2 subsequent plays/i)).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: /cancel/i }))
    expect(onUndo).not.toHaveBeenCalled()
    // Should be back to Edit/Undo buttons
  ...
  it('calls onClose when backdrop clicked', async () => {
    const onClose = vi.fn()
    const user = userEvent.setup()
    const { container } = render(<PlayDetailPopover play={play} onEdit={vi.fn()} onUndo={vi.fn()} onClose={onClose} />)
